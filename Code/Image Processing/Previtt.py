...
for i in range(0,rows):
	for j in range(0,columns):
		...
		if((i!=0)and(i!=rows-1)and(j!=0)and(j!=columns-1)):
			...
			value1= -1*int(gray[i-1,j-1]) + -1*int(gray[i-1,j]) + -1*int(gray[i-1,j+1]) + 1*int(gray[i+1,j-1]) + 1*int(gray[i+1,j]) + 1*int(gray[i+1,j+1])
			
			value2= -1*int(gray[i-1,j-1]) + -1*int(gray[i,j-1]) + -1*int(gray[i+1,j-1]) + 1*int(gray[i-1,j+1]) + 1*int(gray[i,j+1]) + 1*int(gray[i+1,j+1])
...
while(1):
		...
		cv2.imshow('Previtt Mask Edge Detection',output)
