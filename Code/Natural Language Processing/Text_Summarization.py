...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.Message2.place(relx=0.017, rely=0.200, relheight=0.756
                , relwidth=0.955)
...
The simplest kind of summary, an extract, is formed by selecting (extracting) phrases or sentences from the document to be summarized and pasting them together.

Important kinds of summaries are :

1) Outlines of any document
2) Extracts of a scientific article
...
8) Answers to complex questions, constructed by summarizing multiple documents''')
	...
	self.Button1.configure(command=lambda: call(["python3","Text_Summarization/Text_Summarization_main.py"]))
