...
A review of a lecture, book, or notes on the web expresses the student’s sentiment towards the product.
...
Naive Bayes text classification can work well for sentiment analysis. We train naive Bayes classifiers using all words in the training set to estimate positive and negative sentiment.''')
	...
	self.Button1.configure(command=lambda: call(["python3","Sentiment_Analysis/Sentiment_Analysis_main.py"]))
