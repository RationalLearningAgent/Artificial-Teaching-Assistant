...
class Toplevel1:
	...
	self.Button2.configure(command=lambda: call(["python3","Spelling_Correction/Spelling_Checker_using_Trigrams.py"]))
        ...
        self.Message2.place(relx=0.05, rely=0.133, relheight=0.640
                , relwidth=0.922)
        self.Message2.configure(font="-family {DejaVu Sans} -size 10")
        self.Message2.configure(text='''Spelling Correction is a process of detecting and providing suggestions for incorrectly spelled words in a text.

Spell Checker is an application program that flags words in a document that may not be spelled correctly.

When some text is given as an input to spell checker, it list outs the incorrect words separately by checking their availability in the dictionary. Finally it provides the suggestions for the incorrect words from the dictionary.''')
